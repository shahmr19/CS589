...
def binary_metrics(y_true, y_pred, p_label):
    y_true = np.array(y_true)
    y_pred = np.array(y_pred)
    tp = np.sum((y_true == p_label) & (y_pred == p_label))
    tn = np.sum((y_true != p_label) & (y_pred != p_label))
    fp = np.sum((y_true != p_label) & (y_pred == p_label))
    fn = np.sum((y_true == p_label) & (y_pred != p_label))
...
def evaluate(X, y, n_trees, k_folds=10, m_try=None, max_depth=10, min_split=5, min_gain=1e-9, seed=123):
    ...
    for i in range(k_folds):
        ...
        metrics = binary_metrics(y_test.values, np.array(preds), p_label=p_label)
...
def main():
    ...
    parkinsons_path = "../parkinsons.csv"
    max_depth = 10
    min_split = 5
    min_gain = 1e-9

    X_parkinsons, y_parkinsons = load_parkinsons(parkinsons_path)
    experiments(dataset="Parkinsons", X=X_parkinsons, y=y_parkinsons, ntree_vals=ntree_vals, op_prefix="parkinsons", max_depth=max_depth, min_split=min_split, min_gain=min_gain, seed=123)
