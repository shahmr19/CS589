...
def setup():
    ...
    file_path = os.path.join(current_dir, "..", "parkinsons.csv")
...
def runKnn(k, distance, n_splits = 10):
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        train_f1s.append(f1_score_binary(y_train, train_predictions))
        test_f1s.append(f1_score_binary(y_test, test_predictions))
...
def main():
    ...
    plt.xlabel("k")
    plt.ylabel("Accuracy")
    plt.title("Parkinson's k-NN Accuracy vs k using 10-Fold Stratified CV")
    plt.grid(True)
    plt.savefig("parkinsons_knn_accuracy_cv.png")
    plt.close()
    ...
    plt.xlabel("k")
    plt.ylabel("F1-score")
    plt.title("Parkinson's k-NN F1 vs k using 10-Fold Stratified CV")
    plt.grid(True)
    plt.savefig("parkinsons_knn_f1_cv.png")
    plt.close()
